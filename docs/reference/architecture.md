...
| [`src/lib/ocr.ts`](../../frontend/packages/extension/src/lib/ocr.ts) | Side-panel paste / drop / file-picker OCR. Preprocesses (EXIF rotate → downscale ≤ 2,560 px → re-encode JPEG @ 0.92 → HEIC/etc. normalised) then POSTs to `/api/v1/ocr`. Opts into SSE streaming via `Accept: text/event-stream` when the caller supplies an `onPartial` callback. |
| [`src/lib/default-action.ts`](../../frontend/packages/extension/src/lib/default-action.ts) | Picks a default action for newly arriving text: non-English → `translate`; English from a textarea / contenteditable → `grammar`; English from page text or OCR → `reply`. Used by the side panel (selection capture, OCR finish, popover handoff) and by the in-page popover at mount. |
...
| [`src/inkwell_backend/api/v1/ocr.py`](../../backend/src/inkwell_backend/api/v1/ocr.py) | Thin HTTP wrapper over the OCR pipeline; content-negotiates JSON vs SSE on the `Accept` header. |
...
| [`src/inkwell_backend/services/ocr.py`](../../backend/src/inkwell_backend/services/ocr.py) | Validate → rate-limit → cache lookup → call vision model (one-shot or streaming) → write cache → return text. Both `run_ocr` (JSON) and `run_ocr_stream` (SSE) emit exactly one `log.ocr` audit line per terminal outcome. |
| [`src/inkwell_backend/services/ocr_cache.py`](../../backend/src/inkwell_backend/services/ocr_cache.py) | Process-local OCR result cache. TTL+LRU (256 entries, 24 h TTL), keyed by `sha256(model ‖ canonical_base64)`. Repeat OCR of the same image returns instantly without re-billing the upstream. |
...
| [`src/inkwell_backend/services/audit.py`](../../backend/src/inkwell_backend/services/audit.py) | Metadata-only structured logging — propagates `X-Client-Request-Id` for end-to-end correlation; carries `via_portkey` dimension when a real upstream was hit. Two event shapes: `CompletionLogEvent` (chat) and `OcrLogEvent` (OCR — adds `response_chars`, `cache_hit`, `streamed`). |
