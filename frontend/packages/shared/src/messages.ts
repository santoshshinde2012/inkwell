...
export const MESSAGE_TYPES = {
  ...
  // Background -> Content: the right-click "Extract text with Inkwell"
  // context menu fired. Sent synchronously BEFORE OCR runs so the
  // content script can show a fixed-position loader card in the page,
  // giving the user immediate feedback while the (1-3 s) OCR pipeline
  // executes in the background.
  OPEN_OCR_LOADER: "OPEN_OCR_LOADER",

  // Background -> Content: the right-click "Extract text with Inkwell"
  // context menu finished OCR. The content script removes any pending
  // loader and opens the in-page popover in selection mode, pre-filled
  // with the extracted text — or with the error message when OCR
  // failed. Payload always carries either `text` or `errorMessage`,
  // never both.
  OPEN_OCR_POPOVER: "OPEN_OCR_POPOVER",
} as const;
...
export type CheckSiteAllowedMessage = z.infer<typeof CheckSiteAllowedMessageSchema>;

export const CheckSiteAllowedResponseSchema = z.object({
  allowed: z.boolean(),
  reason: z.enum(["default", "allowlist", "blocklist", "blocked-by-default"]),
});
export type CheckSiteAllowedResponse = z.infer<typeof CheckSiteAllowedResponseSchema>;

// ---- Background → content: right-click OCR lifecycle ----------------------

export const OpenOcrLoaderMessageSchema = z.object({
  type: z.literal(MESSAGE_TYPES.OPEN_OCR_LOADER),
});
export type OpenOcrLoaderMessage = z.infer<typeof OpenOcrLoaderMessageSchema>;

// Plain object — no `.refine()` here because discriminatedUnion requires
// a `ZodObject`, not the `ZodEffects` that .refine produces. The handler
// in the content script enforces "exactly one of text / errorMessage".
export const OpenOcrPopoverMessageSchema = z.object({
  type: z.literal(MESSAGE_TYPES.OPEN_OCR_POPOVER),
  /** The OCR'd text, when recognition succeeded. */
  text: z.string().max(50_000).optional(),
  /** A user-facing failure message, when OCR couldn't complete. */
  errorMessage: z.string().max(1000).optional(),
});
export type OpenOcrPopoverMessage = z.infer<typeof OpenOcrPopoverMessageSchema>;
...
export type OpenSidePanelFromPopoverMessage = z.infer<typeof OpenSidePanelFromPopoverMessageSchema>;
...
export const ExtensionMessageSchema = z.discriminatedUnion("type", [
  ...
  OpenOcrLoaderMessageSchema,
  OpenOcrPopoverMessageSchema,
]);
