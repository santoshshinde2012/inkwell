...
export const LANGUAGE_IDS = LANGUAGE_CATALOG.map((l) => l.id) as [LanguageId, ...LanguageId[]];
...
export const SOURCE_LANGUAGE_IDS = [AUTO_DETECT, ...LANGUAGE_IDS] as [AutoDetect, ...LanguageId[]];
...
export const languageLabel = (id: string): string => getLanguageInfo(id)?.label ?? id;
...
export const languageDisplayName = (id: string): string => {
  ...
  if (!info) return id;
  return info.label === info.nativeName ? info.label : `${info.label} (${info.nativeName})`;
};
