...
export const localStore = {
  ...
  async setWorkingLanguage(language: LocalSettings["workingLanguage"]): Promise<void> {
    await chrome.storage.local.set({ [KEYS.workingLanguage]: language });
  },

  async setFrequentLanguages(languages: LocalSettings["frequentLanguages"]): Promise<void> {
    await chrome.storage.local.set({
