...
import { ExtensionMessage, ExtensionMessageSchema } from "@inkwell/shared";
...
export class ExtensionContextInvalidatedError extends Error {
  constructor() {
    super("Inkwell was updated or reloaded. Refresh this page to continue.");
...
const isContextInvalidatedMessage = (msg: string | undefined): boolean =>
  typeof msg === "string" && msg.toLowerCase().includes(CONTEXT_INVALIDATED_HINT);
...
export const sendToBackground = <R = unknown>(msg: ExtensionMessage): Promise<R> => {
