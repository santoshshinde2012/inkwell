...
export class GmailAdapter implements SiteAdapter {
  ...
  async extractContext(target: HTMLElement): Promise<RequestContext> {
    ...
    const subject =
      document.querySelector("h2[data-thread-perm-id]")?.textContent?.trim() ?? document.title;
  ...
  private extractThread(): RequestContext["thread"] {
    ...
    for (const c of list) {
      const author =
        ...
        undefined;
      const body = c.querySelector<HTMLElement>("div.a3s")?.innerText.trim() ?? c.innerText.trim();
      if (!body) continue;
      out.push({
        author,
