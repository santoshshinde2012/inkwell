...
export class OutlookAdapter implements SiteAdapter {
  ...
  async extractContext(target: HTMLElement): Promise<RequestContext> {
    ...
    const subject =
      document.querySelector<HTMLElement>('[role="heading"][aria-level="2"]')?.innerText.trim() ??
      document.querySelector<HTMLElement>('[role="main"] [role="heading"]')?.innerText.trim();
  ...
  private extractThread(): NonNullable<RequestContext["thread"]> {
    ...
    for (const body of Array.from(bodies).slice(-MAX_THREAD_MESSAGES)) {
      ...
      const card = body.closest<HTMLElement>('[role="listitem"], article') ?? body;
      const author =
        card.querySelector<HTMLElement>("span[title*='@']")?.getAttribute("title")?.trim() ??
        card.querySelector<HTMLElement>('[aria-label*="From" i]')?.innerText.trim();
      out.push({
